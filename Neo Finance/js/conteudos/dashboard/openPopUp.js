...
function carregarCategorias() {
  fetch("../../../config/conteudos/dashboard/carregar_categorias.php") // Altere o caminho conforme necessário
    .then((response) => response.json())
    .then((data) => {
      if (data.status === "success") {
        var select = document.getElementById("categoria--select");
        select.innerHTML = ""; // Limpa o select antes de adicionar as opções

        data.categorias.forEach((categoria) => {
          var option = document.createElement("option");
          option.value = categoria.id; // ID da categoria
          option.text = categoria.nome; // Nome da categoria
          select.add(option);
        });
      } else {
...
document.getElementById("btn--abrir--popup").addEventListener("click", function () {
  carregarCategorias(); // Carrega as categorias ao abrir o popup
...
// Função para abrir o popup de seleção de ícones
