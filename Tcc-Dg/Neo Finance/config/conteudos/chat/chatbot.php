<?php
include("../../database/conect.php"); // Inclui a conexão com o banco de dados
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $responses = [
        ...
        'saldo' => function() use ($mysqli, $userId) {
            return getSaldo($mysqli, $userId);
        },
        '1' => function() use ($mysqli, $userId) {
            return getSaldo($mysqli, $userId);
        },
        'categorias' => function() use ($mysqli, $userId) {
            return getCategorias($mysqli, $userId);
        },
        '2' => function() use ($mysqli, $userId) {
            return getCategorias($mysqli, $userId);
        },
        'economizar' => function() use ($mysqli, $userId) {
            return implode("\n", getDicasEconomizar($mysqli, $userId));
        },
        '3' => function() use ($mysqli, $userId) {
            return implode("\n", getDicasEconomizar($mysqli, $userId));
        },
        'investir' => function() use ($mysqli, $userId) {
            return implode("\n", getDicasInvestir($mysqli, $userId));
        },
        '4' => function() use ($mysqli, $userId) {
            return implode("\n", getDicasInvestir($mysqli, $userId));
        },
        'resumo mensal' => function() use ($mysqli, $userId) {
            return getResumoMensal($mysqli, $userId);
        },
        '5' => function() use ($mysqli, $userId) {
            return getResumoMensal($mysqli, $userId);
        },
        'resumo diário' => function() use ($mysqli, $userId) {
            return getResumoDiario($mysqli, $userId);
        },
        '6' => function() use ($mysqli, $userId) {
            return getResumoDiario($mysqli, $userId);
        },
        'Historico' => function() use ($mysqli, $userId) {
            return getHistoricoTransacoes($mysqli, $userId);
        },
        '7' => function() use ($mysqli, $userId) {
            return getHistoricoTransacoes($mysqli, $userId);
        },
        'Análise' => function() use ($mysqli, $userId) {
            return getAnaliseGastos($mysqli, $userId);
        }, 
        '8' => function() use ($mysqli, $userId) {
            return getAnaliseGastos($mysqli, $userId);
        },
         'Exportar' => function() use ($mysqli, $userId) {
            return exportarRelatorio($mysqli, $userId);
        },       
        '9' => function() use ($mysqli, $userId) {
            return exportarRelatorio($mysqli, $userId);
        },
        'Previsão financeira' => function() use ($mysqli, $userId) {
            return previsaoFinanceira($mysqli, $userId);
        },
        '10' => function() use ($mysqli, $userId) {
            return previsaoFinanceira($mysqli, $userId);
        },
        'Comparação' => function() use ($mysqli, $userId) {
            return comparacaoGastosMensais($mysqli, $userId);
        },
        '11' => function() use ($mysqli, $userId) {
            return comparacaoGastosMensais($mysqli, $userId);
        },
        'Desafio' => function() use ($mysqli, $userId) {
            return obterDesafioFinanceiroAleatorio($mysqli, $userId);
        },
        '12' => function() use ($mysqli, $userId) {
            return obterDesafioFinanceiroAleatorio($mysqli, $userId);
        },
    ...
    $synonyms = [
        'oi', 
        'saldo', 'quanto tenho', 'meu saldo', 'saldo atual',
        'categorias', 'quais são minhas categorias', 'minhas categorias',
        'economizar', 'dicas de economia', 'como economizar',
        'investir', 'dicas de investimento', 'como investir',
        'resumo mensal', 'meu resumo mensal', 'resumo do mês',
        'resumo diário', 'meu resumo diário', 'resumo de hoje',
        'ajuda', 'preciso de ajuda', 'me ajuda',
        'como funciona', 'como isso funciona', 'me explique'
    ];
    ...
    $stmt = $mysqli->prepare("INSERT INTO historico (usuario_id, mensagem, resposta) VALUES (?, ?, ?)");
...
function getSaldo($mysqli, $userId) {
    // Somar receitas
    $stmtReceitas = $mysqli->prepare("SELECT SUM(valor) FROM transacoes WHERE usuario_id = ? AND tipo = 'receita'");
    ...
    // Somar despesas
    $stmtDespesas = $mysqli->prepare("SELECT SUM(valor) FROM transacoes WHERE usuario_id = ? AND tipo = 'despesa'");
...
function getCategorias($mysqli, $userId) {
...
function getDicasEconomizar($mysqli, $userId) {
    // Obter o saldo atual do usuário
    $stmt = $mysqli->prepare("SELECT SUM(valor) FROM transacoes WHERE usuario_id = ?");
    $stmt->bind_param("i", $userId);
    $stmt->execute();
...
function getDicasInvestir($mysqli, $userId) {
...
function getResumoMensal($mysqli, $userId) {
    $stmt = $mysqli->prepare("SELECT SUM(valor) FROM transacoes WHERE usuario_id = ? AND MONTH(data) = MONTH(CURRENT_DATE())");
    $stmt->bind_param("i", $userId);
    $stmt->execute();
...
function getResumoDiario($mysqli, $userId) {
    $stmt = $mysqli->prepare("SELECT SUM(valor) FROM transacoes WHERE usuario_id = ? AND DATE(data) = CURDATE()");
    $stmt->bind_param("i", $userId);
    $stmt->execute();
...
function getHistoricoTransacoes($mysqli, $userId) {
    $stmt = $mysqli->prepare("SELECT tipo, valor, data FROM transacoes WHERE usuario_id = ? ORDER BY data DESC");
    $stmt->bind_param("i", $userId);
    $stmt->execute();
...
function getAnaliseGastos($mysqli, $userId) {
    $stmt = $mysqli->prepare("SELECT c.nome, SUM(t.valor) AS total FROM transacoes t JOIN categorias c ON t.categoria_id = c.id WHERE t.usuario_id = ? GROUP BY c.nome");
    $stmt->bind_param("i", $userId);
    $stmt->execute();
...
function exportarRelatorio($mysqli, $userId) {
    $stmt = $mysqli->prepare("SELECT username FROM users WHERE id = ?");
    $stmt->bind_param("i", $userId);
    $stmt->execute();
    $stmt->bind_result($username);
    ...
    $historico = getHistoricoTransacoes($mysqli, $userId);
...
function previsaoFinanceira($mysqli, $userId, $meses = 3) {
    $stmt = $mysqli->prepare("
...
function comparacaoGastosMensais($mysqli, $userId) {
    $stmt = $mysqli->prepare("
...
function obterDesafioFinanceiroAleatorio($mysqli, $userId) {
    $desafios = [
